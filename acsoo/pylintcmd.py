...
from .tools import cfg_path, cmd_string, log_cmd
...
def _get_failures(linter_stats, expected):
    ...
    return (sorted(fails, key=lambda i: i[0]), sorted(no_fails, key=lambda i: i[0]))
...
def _failures_to_str(fails, no_fails):
    ...
    res = []
    if no_fails:
        res.append(click.style("messages that did not cause failure:\n", bold=True))
        _r(no_fails)
    if fails:
        res.append(click.style("messages that caused failure:\n", bold=True))
        _r(fails)
    return "".join(res)
...
def do_pylintcmd(load_plugins, rcfile, module, expected, pylint_options):
    ...
    if not module:
        ...
        module.extend(addon_dir for addon_name, (addon_dir, manifest) in addons.items())
        if not module:
            raise click.UsageError(
                "Please provide module or package " "to lint (--module)."
            )
    cmd = (
        ["--load-plugins", load_plugins, "--rcfile", rcfile]
        + list(pylint_options)
        + list(module)
    )
...
def pylintcmd(ctx, load_plugins, rcfile, module, expected, pylint_options):
    ...
    default_pylint_options = (ctx.default_map or {}).get("default_pylint_options", [])
...
def _read_defaults(config):
    section = "pylint"
    defaults = dict(
        module=config.getlist(section, "module"),
        load_plugins=config.get(section, "load-plugins", default="pylint_odoo"),
        rcfile=config.get(section, "rcfile", default=cfg_path("pylint.cfg")),
        expected=config.get(section, "expected", flatten=True),
        default_pylint_options=config.getlist(section, "pylint-options"),
    )
